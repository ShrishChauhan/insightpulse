...
- Reddit permanently closed all API access May 30 2026. Replaced with Bluesky (social), ProductHunt (launch discourse), App Store RSS (direct app reviews), HN, RSS. YouTube removed — Google Cloud billing blocked in India via UPI, so it was never operational.
- Bluesky public.api.bsky.app/xrpc/app.bsky.feed.searchPosts returns 403 from non-residential IPs. Fix: POST com.atproto.server.createSession with BLUESKY_HANDLE + BLUESKY_APP_PASSWORD to get accessJwt; inject as Authorization: Bearer header. Gracefully skipped if credentials absent.
...
- Zernio (zernio.com) has pre-approved w_organization_social scope from LinkedIn. POST zernio.com/api/v1/posts with just {content, platforms:[{platform,accountId}], publishNow:true} — do NOT include platformSpecificData.organizationUrn when the Zernio account itself is the org (accountType=organization); that field is only for personal accounts posting to a managed page and causes a 400 otherwise. ZERNIO_ACCOUNT_ID from GET /api/v1/accounts _id field. Return status="posted" (not "published") to match orchestrator's success check.
- Automation runs via GitHub Actions (.github/workflows/). Secrets stored in GitHub repo Settings → Secrets → Actions. Test workflows manually via GitHub UI → Actions → Run workflow.
