...
- Zernio (zernio.com) has pre-approved LinkedIn Community Management API access. Use it to post to LinkedIn company pages without LinkedIn's own approval process. Free tier: 10 posts/month. Endpoint: POST zernio.com/api/v1/posts with organizationUrn in platformSpecificData. Priority order in linkedin_poster.py: Zernio (if ZERNIO_API_KEY+ZERNIO_ACCOUNT_ID set) > LinkedIn API org page > LinkedIn API personal profile.
